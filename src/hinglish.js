...
export function generateHinglishSummary(match, players, ballLog, inningsSummaries) {
  ...
  for (const summary of inningsSummaries) {
    ...
    // Per-player one-liners for this innings
    lines.push('');
    lines.push(`👥 **Player Report Card:**`);
    for (const [id, s] of Object.entries(batsmanStats)) {
      const player = players.find(p => p.id === id);
      if (!player) continue;
      lines.push(getHinglishPlayerVerdict(player.name, s));
    }
...
function getHinglishPlayerVerdict(name, stats) {
  const { runs, balls, fours, sixes, out } = stats;
  const sr = balls > 0 ? ((runs / balls) * 100).toFixed(0) : 0;

  if (runs === 0 && out) {
    return pickRandom([
      `  🦆 **${name}**: Zero pe out — aaj toh bilkul off day tha bhai!`,
      `  🦆 **${name}**: Bat ghuma ke bhi nahi laga — better luck next time!`,
    ]);
  }
  if (runs >= 50) {
    return pickRandom([
      `  🌟 **${name}**: ${runs}(${balls}) — Kya innings thi! Sab ke muh band kara diye! 🔥`,
      `  💥 **${name}**: Half-century wala ${runs}(${balls}) — full paisa wasool!`,
    ]);
  }
  if (runs >= 30) {
    return pickRandom([
      `  💪 **${name}**: ${runs}(${balls}) — Achha contribution, team ke liye kaam kiya!`,
      `  ⚡ **${name}**: ${runs}(${balls}) — Solid innings, strike rate bhi ${sr} tha!`,
    ]);
  }
  if (runs >= 15) {
    return pickRandom([
      `  ✅ **${name}**: ${runs}(${balls}) — Theek-thaak khela, kaam chala diya.`,
      `  👍 **${name}**: ${runs}(${balls}) — Supporting role mein achha raha.`,
    ]);
  }
  if (runs > 0) {
    return `  📝 **${name}**: ${runs}(${balls}) — Thoda aur time ruk lete toh achha hota.`;
  }
  return `  📝 **${name}**: DNB ya phir kuch khaas nahi hua.`;
}
